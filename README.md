...
1. A mathly table is a simple Lua table registered as a mathly object. E.g., `x = tt{1, 2, 3}` is such a table. It has exactly the same structure as an ordinary Lua table `y = {1, 2, 3}`. The difference is that we can apply "vectorization" operations on `x` instead of `y`. For instance, `2 * x - 1` gives a new mathly table, {1, 3, 5}. Besides, `x[i]` gives the i-th element in the table.
