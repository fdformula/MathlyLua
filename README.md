# Mathly Turns Lua into a Tiny, Free but Powerful MATLAB and More

Mathly for [Lua](https://www.lua.org) is a Lua module which turns Lua into a tiny, free but powerful MATLAB and more. It provides a group of commonly
...
Linux users? For most Linux distributions, download, say, cudatext-for-mathly-linux-1.224.0.0.tar.gz. For other distributions like Fedora, download cudatext-for-mathly-linux-1.224.0.0-RARE.tar.gz. Expand the file and refer to the included file, note.txt. Please be advised that the included CudaText may not work because it is the gtk2 version. In that case, download the versy CudaText version you need in https://cudatext.github.io/ and replace the file, usr/share/cudatext/cudatext, with it.
...
`isvector`, `lagrangepoly`, `length`, `linsolve`, `linspace`, `lu`, `map`, `match`, `mathly`, `max`, `mean`, `merge`, `min`, `mod`, `mtable`, `namedargs`, `newtonpoly`, `norm`,
