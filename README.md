...
Windows users may download on this very page the file, `cudatext-for-mathly-public-1_220_6_1.7z`. Run [7zip](https://7-zip.org/) to
extract it to C:/ . It comes with Lua 5.4.6. CudaText is a very good "IDE" for Lua and running mathly as well.
...
`all`, `any`, `apply`, `cc`, `check`, `clc`, `clear`, `copy`, `cross`, `det`, `diag`, `disp`, `display`, `dot`, `expand`, `extract`, `eye`,
...
`reshape`, `reverse`, `rr`, `rref`, `save`, `seq`, `size`, `sort`, `sprintf`, `std`, `strcat`, `submatrix`, `subtable`, `sum`, `tblcat`,
