...
-- explore a folder and all subfolders for files/folders specified in 'specs'
--
-- 1. no change is needed for function explore(...)
-- 2. edit function process(...) at the bottom of this file for your task
--
-- opts.folder    == true:  process matched folder and subfolders only
-- opts.folder    == false: process matched files only (default)
-- opts.subfolder == true:  process matched items in subfolders (default)
-- opts.subfolder == false: process matched items in 'dir' only
--
-- opts.task: function(fname, i, n) that defines how to process 'fname', the
-- i-th one of all n files (or folders)
--
