...
LUA MODULE
  ...
  mathly v$(_VERSION) - table functions that make doing math faster
...
DESCRIPTION
  ...
  Two three important things are:
...
API
  ...
  function diag
  function disp             print a matrix
  function display          print a table with structures
  function flatten
  function length
  function reshape
  function size
  ...
  function ismember
  function ls
  function save      -- save data to file ('dofile' to restore)

  function printf    -- as in C/C++
  function sprintf

  See code and examples in mathly.lua.

DEPENDENCIES

  plotly: plotly.lua, plotly-2.9.0.min.js
...
USAGE
  ...
  To use features of vertor/matrix operations, imagine that we need all vector/matrix objects. They are
  generated or converted from given 'raw' tables, for example,

    a = mathly{{1,2},{3,5}}
    b = mathly({{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}, 4)
    c = mathly({{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}, 4, 5)
    d = mathly(5, 5)
    f = mathly(5, 5, 100)

  We may convert a 'raw' LUA table to a vector/matrix object as follows:

    a = {{1,2},{3,5}}; setmetatable(a, mathly_meta)

  Because matrices, generated by eye, ones, zeros, rand, and randi, are all for use in matrix operations,
  they are naturally vector/matrix objects.


--]]

-- ↓ ↓ ↓ mathly makes Lua more mathly -- ↓ ↓ ↓ --

--[[ no help because only t and i can be passed to the function !
local mathly_meta = {
__index = function(t, i, j)
  if #t[1] == 1 then j = 1 end
  print(i, j)
  assert(i > 0 and i <= #t and j > 0 and j <= #t[1],
         sprintf('Out of bound (dimensions are %dx%d)', #t, #t[1]))
  if #t == 1 then i, j = 1, i end
  return t[i][j]
end
}
--]]

local mathly = {_TYPE='module', _NAME='mathly', _VERSION='0.12.19.202412.19'}
...
--[[
    Code borrow from matrix.lua, https://github.com/davidm/lua-matrix/blob/master/lua/matrix.lua
--]]
--// mathly.det ( m1 )
...
function det( m1 )
  assert(getmetatable(m1) == mathly_meta, 'det( m ): m must be a mathly metatable.')
	if #m1 ~= #m1[1] then print("matrix not square"); return 0 end -- assert(#m1 == #m1[1])
	local size = #m1
...
-- ↑ ↑ ↑ mathly makes Lua more mathly -- ↑ ↑ ↑ --
-- David Wang, dwang@liberty.edu, on 12/18/2024 Wednesday
